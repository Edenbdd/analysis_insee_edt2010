...
for d in data:
    ...
    int_tot.append(mean)
for d in data:
    mean = d['broad_tot'].mean()
    broad_tot.append(mean)
...
# Tracé des barres
ax.bar(x1, res_tot, width=bar_width, label='périmètre restreint')
ax.bar(x2, int_tot, width=bar_width, label='périmètre intermédiaire')
ax.bar(x3, broad_tot, width=bar_width, label='périmètre élargi')
