...
men = men[["IDMEN", "RG", "NENFANTS", "NENFHORS"]]
...
print(len(merged))
old = merged.loc[(merged['age'] >= 25) & (merged['age'] <= 65) & (~merged['RG'].isin([1, 2, 3, 4])) & 
                 (merged['couplrp'] == 1) & ((merged['NENFANTS'] != 0) | (merged['NENFHORS'] != 0))]
print(len(old))
old.to_csv("echantillonchildren.csv")